...
def apply_change(change,graph,rate_of_antiv):
	anti=int(len(change)*rate_of_antiv)
	for i in range(0,anti):
		query= "MATCH (n {name : %d}) where n.stat='S' SET n.stat='I' set n.d_t=n.d_t+2 " % change[i]
		result=graph.cypher.execute(query)
	for i in range(anti+1,len(change)):
